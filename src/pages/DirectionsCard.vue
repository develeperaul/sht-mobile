<template>
  <q-page class="tw-container bg">
      ...
      <div v-else>
        <template v-if="direction.data">
          <div class="tw-mb-[18px]">
            ...
            <div class="p1 tw-text-white">{{ direction.data.title }}</div>
          </div>
          <div class="tw-mb-5">
          ...
          <div>
            ...
            <div class="tw-grid tw-gap-3">
              <div class="tw-bg-white tw-p-5 tw-rounded-32">
                <div v-if="currentOffer.data" class="tw-grid tw-gap-3 tw-mb-6">
                  <div class="h3">Стоимость</div>
                  <div
                    class="h2"
                    v-amount-pretty="Number(currentOffer.data.price).toFixed(0)"
                  ></div>
                </div>
                <div
                  v-if="
                    direction.data && direction.data.preview_stories.length > 0
                  "
                >
    ...
    <div
      v-if="selectOffer && options.length > 0"
      class="tw-fixed tw-bottom-[100px] tw-left-0 tw-right-0 tw-z-10 tw-container"
    >
      <BaseButton
        @click="router.push({ name: 'booking', params: { uuid: selectOffer } })"
...
const { direction, offersMap, offers, currentOffer, transportData, powerData } =
...
const options = computed(() => {
  if (offers.value) {
    return offers.value.map((of) => {
      return {
        id: of.id,
        name: `${dayjs(of.start_date).locale('ru').format('DD MMMM')}-${dayjs(of.end_date).locale('ru').format('DD MMMM')} (мало мест)`,
      }
    })
  }
  return []
...
watch(tab2, async (v, oV) => {
  ...
  if (tab.value) {
    if (!offersMap.value.get(keyMap.value)) {
      try {
        await directionsStore().setOffers(props.id, tab.value, v)
      } catch (e) {
        throw e
      }
    }
