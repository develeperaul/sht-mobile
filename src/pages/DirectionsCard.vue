    ...
    <div v-else>
      <template
        v-if="
          ...
          directionsSubgroup.data.data.length > 1 &&
...
onMounted(async () => {
  try {
    ...
    if (
      directionsSubgroup.value.data?.dates &&
      directionsSubgroup.value.data?.dates.length > 1
    ) {
      isSubgroup.value = true
      tabs2.value = directionsSubgroup.value.data.dates.map((day) => {
        return {
          id: day,
          name: day,
        }
...
const formatDate = (d: { id: string; name: string }) => {
  ...
  if (diffLocale >= 1) return dayjs(d.id).locale('ru').format('YYYY MMMM')
